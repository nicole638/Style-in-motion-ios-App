import { decodeHtmlEntities } from "./decode-entities.ts";
...
export function parseProductMetadata(html: string, sourceUrl: string = ''): ParsedProductMetadata {
  ...
  // iOS App Transport Security blocks http:// images. Modern retailer CDNs
  // (Shopify, Cloudinary, etc.) all serve the same asset over HTTPS, so
  // coerce the scheme — otherwise the image area renders blank in the app.
  if (imageUrl) {
  ...
  if (imageUrl) {
    ...
    const rest = candidates.filter(u => normalizeImageKey(u) !== key);
