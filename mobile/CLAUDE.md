...
<mistakes>
  ...
  <buttons>
    Every interactive button MUST be a fully-rounded pill styled via NativeWind className — never via StyleSheet.create. StyleSheet entries on `<Pressable>` (especially with the function-form `style={({pressed}) => [...]}`) do not render reliably in this build config and have repeatedly produced invisible buttons.

    SiM brand button pattern (use these classNames):
      Primary (coral, leading action):
        className="bg-[#B87063] rounded-full py-3.5 px-5 flex-row items-center justify-center active:opacity-85"
        Inner Text className="ml-2 text-white text-[15px] font-semibold"
        For elevated emphasis add the iOS shadow inline:
          style={{ shadowColor: '#1A1210', shadowOpacity: 0.12, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 }}

      Secondary (white pill with dark border, supporting action):
        ...
        Inner Text className="ml-2 text-[#1A1210] text-[15px] font-semibold"

    Always use `rounded-full` (or `rounded-2xl` if a subtle pill is required by context). NEVER ship a button as bare text+icon without a visible background — if the design calls for a tertiary text-only action, give it `py-2 px-3` and a tap target ≥ 44px tall.

    Tertiary (text-only, low-emphasis):
        className="flex-row items-center justify-center gap-1.5 py-2 px-3 active:opacity-70"
        Inner Text className="text-[#6B5E58] text-sm font-medium"

    Font: Text inside buttons should set `style={{ fontFamily: 'DMSans_500Medium' }}` (NativeWind doesn't compile custom font families) — that's the only acceptable use of `style` on a button child Text.

    When in doubt, copy the existing pill pattern from `mobile/src/app/try-on-flow.tsx` (Take Photo / Choose from Library) or `mobile/src/components/ItemListSheet.tsx` (Try It On).
  </buttons>
