...
<schema_discipline>
  All DB schema changes go through `supabase/migrations/`. No Studio edits to prod without a paired migration file. CI fails if `supabase db diff` returns non-empty. RLS policies count as schema. See `docs/schema-discipline.md` for the full rule and rationale.
</schema_discipline>
