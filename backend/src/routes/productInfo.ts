...
import { parseProductMetadata, isGoodEnough, normalizeImageKey, type ParsedProductMetadata } from "../lib/parseProductMetadata";
import {
  getScrapingBeeMode,
...
import { normalizeUrlInput, resolveShortUrl } from "../lib/normalizeUrlInput";
import { cacheMerchantImage } from "../lib/cacheMerchantImage";
import { fetchShopifyGalleryUrls, mergeGalleryIntoCandidates } from "../lib/shopifyGallery";

const productInfoRouter = new Hono();
 ...
 * /dp/<ASIN> or /gp/product/<ASIN> URL. Strips ALL query parameters — including
 * `ref`, `campaignId`, `tag`, `linkCode`, `ascsubtag`, etc. — because Amazon
 * sometimes serves a stripped-down / bot-checked variant when those params are
 * present, which causes the parser to fall back to the title only and miss
 * imageUrl/price/brand entirely.
 *
 * Behavior:
 * - If the host is not Amazon, returns the input URL unchanged.
 * - If the host IS Amazon but no ASIN is extractable, returns the input URL
 *   unchanged (so we don't break a.co short URLs that haven't been resolved
 *   yet, or storefront links that lack an ASIN).
 * - Always returns the canonical https://www.amazon.com host so we don't
 *   accidentally hit a regional variant via a redirect chain.
 *
...
// populate the response correctly. Used to enrich the parser output when
// the host is Amazon.
 ...
 * extractors so the result has name/imageUrl/brand/price populated. Amazon
 * does not expose JSON-LD or OG metadata on product pages so without this we
 * fall through to title-fallback with image/price/brand null.
 *
...
function parseAmazonAware(html: string, url: string): ParsedProductMetadata {
  ...
  // value; otherwise keep whatever the generic parser found. This way an
  // unusual Amazon page that DOES happen to have og: tags still benefits
  // from them when our DOM extractor misses.
  const finalImage = amzImage || base.imageUrl;
  // Keep imageUrls[0] aligned with imageUrl when the Amazon extractor
  // overrides. Amazon pages don't expose multi-image metadata via JSON-LD/OG
  // so the candidates list is typically empty here anyway.
 ...
 * flows into siteName/canonicalUrl derivation. For Amazon the route passes the
 * original inbound URL (with tracking params) as displayUrl so response shape
 * is unaffected by the FETCH-time canonicalization.
 */
async function fetchDirect(fetchUrl: string, displayUrl: string = fetchUrl): Promise<DirectFetchOutcome> {
  ...
  try {
    ...
    // For non-OK responses we still try to inspect body so we can pick up
    // challenge titles even when the server signals 403/429.
    let body = '';
    if (isHtmlish || !res.ok) {
...
async function fetchViaScrapingBee(fetchUrl: string, displayUrl: string = fetchUrl): Promise<{ result: ProductInfoResult; httpStatus: number | null }> {
  ...
  const tryFetch = async (renderJs: boolean): Promise<{ result: ProductInfoResult; httpStatus: number | null } | null> => {
    ...
    try {
      // 25s timeout — without this, hung ScrapingBee fetches stall the
      // entire Hono request, and the platform proxy returns 502 to the
      // client (observed for slow merchants like Macy's premium fetches).
...
/**
 * Core scrape cascade, extracted from the GET "/" handler so it can be reused by
 * the batch ASIN resolver. Takes a raw (already-normalized) URL, resolves any
 ...
 * Amazon metadata attached). The single-URL GET "/" handler and the batch path
 * both go through here so their scraping behavior stays identical.
 */
export async function resolveProductInfo(rawUrl: string): Promise<ProductInfoResult> {
  ...
  // Amazon hosts: preserve tracking params + extract ASIN. Apply this to every
  // result returned from this handler. For non-Amazon hosts, results pass
  // through unchanged (no metadata field).
  ...
  // (or /gp/product/<ASIN>) link, fetch the BARE canonical URL only, stripping
  // ref/campaignId/tag/linkCode/etc. Amazon serves a stripped-down /
  // bot-checked variant for some referral combinations (e.g. ref=t_ac_*) that
  // collapses our parser to title-fallback with empty image/price/brand. The
  // response's canonicalUrl + metadata are still derived from `url` (the
  // ORIGINAL inbound URL) so tracking params are not lost in the response.
  ...
  //    Note: getScrapingBeeMode is keyed by host, so checking it on `url` vs
  //    `fetchUrl` is equivalent (they have the same host on Amazon, and
  //    fetchUrl === url on non-Amazon hosts).
...
// (PA-API → Bright Data Web Unlocker → Microlink). Bright Data reliably pulls
// the /dp page but is slower per ASIN (~5–20s) than the old inline ScrapingBee
// scraper, which Amazon was CAPTCHA-ing anyway. Budget is sized for ONE EF
// round-trip, not N: the EF fans its ASINs out in parallel internally, so a
// 10-ASIN chunk costs roughly one Web Unlocker latency, not 10×.
...
/**
 * Parse the scraper's price string ("$1,299.99") into a numeric value. Strips
 * the leading currency symbol and thousands separators. Returns null when the
 * input is missing or doesn't contain a parseable number.
 */
 ...
 * result into `amazon_product_cache` BEFORE responding, so the caller can read
 * the rows back immediately. Returns the EF's `via` provider counts (for
 * observability) or null on timeout / error — fail-soft, the caller just leaves
 * those ASINs unresolved to retry next view. We do NOT touch the EF itself.
 */
 ...
 * return only the rows the EF has marked `complete` with usable data. Used both
 * to pick up rows a prior view already enriched (no EF call needed) and to read
 * back a chunk right after the EF write.
 */
 ...
 * so subsequent batch calls serve them instantly from the bulk cache read,
 * without re-hitting amazon_product_cache or the EF. Only rows with an image are
...
async function handleAsinBatch(c: Context) {
  const asins = parseAsinsParam(c.req.query("asins"));
  if (asins.length === 0) {
    return c.json(
  ...
  // 3) Resolve misses via the enrich-amazon-asin EF (Bright Data tier), bounded
  //    by an overall wall-clock so a cold batch can finish without hanging.
  let scrapedCount = 0;
  const via: EnrichVia = { paapi: 0, brightdata: 0, microlink: 0 };
  if (toScrape.length > 0) {
    const deadlineAt = Date.now() + BATCH_WALL_CLOCK_MS;

    // 3a) Some misses may already be `complete` in amazon_product_cache (the
    //     EF's output table) from a prior view — use those with no EF call, and
    //     mirror them into the primary cache.
...
productInfoRouter.get("/", async (c) => {
  // Batch ASIN branch: when `?asins=` is present, delegate to the batch resolver
  // and return immediately so the existing `?url=` behavior is untouched.
  if (c.req.query("asins") !== undefined) {
    return handleAsinBatch(c);
  }

  const rawUrl = c.req.query("url");
  const normalized = normalizeUrlInput(rawUrl);
  if (!normalized) {
    return c.json({ error: { message: "url query param is required", code: "VALIDATION_ERROR" } }, 400);
  }
