...
const shopRedirectRouter = new Hono();
...
// pass — so none of that credential logic lives in Hono.
//
...
// apikey (we use the service-role key the backend already holds).
...
shopRedirectRouter.get('/', async (c) => {
  const lookId = c.req.query('lookId');
  ...
  const creatorItemId = c.req.query('creatorItemId');
  const urlParam = c.req.query('url');
  const creatorIdParam = c.req.query('creatorId') ?? null;
  ...
  const srcRaw = c.req.query('source') ?? c.req.query('src');
  const referer = c.req.header('referer') ?? null;
  const userAgent = c.req.header('user-agent') ?? null;
  // Allow-list: 'ios' (mobile), 'web' (shop.styledinmotion.studio), 'android'
  // (future), 'creator' (creators-web — Kerri clicking from her own dashboard
  // so analytics can filter out self-traffic). Anything else is ignored and
  // inferred from referer / UA below.
  ...
  if (lookId && itemId) {
    ...
    if (!host) {
      return c.json({ error: { message: 'Invalid url', code: 'VALIDATION_ERROR' } }, 400);
    }
  ...
  if (amazon) {
    ...
    const resolved = resolveAmazonTag({
      ...
      masterTag: env.AMAZON_PLATFORM_ASSOCIATES_TAG,
    });
    if (resolved) {
  ...
  } else if (rakutenRaw && env.RAKUTEN_PUBLISHER_ID) {
